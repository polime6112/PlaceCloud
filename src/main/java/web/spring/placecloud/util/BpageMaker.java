...
public class BpageMaker {
	private Bpagination bpagination; // 페이징 처리 객체
	...
	private int calcTotalPageNum() {
		// Math.ceil (올림)
		// Math.floor (버림)		
		return (int) Math.ceil((double) totalCount / bpagination.getPageSize()); 
		
	} // end calcTotalPageNum()
	
	// 임시 끝 번호 계산값
	private int tempEndNum() {
		return (int) Math.ceil((double) bpagination.getPageNum() / pageCount) * pageCount;
	} // end tempEndNum()
