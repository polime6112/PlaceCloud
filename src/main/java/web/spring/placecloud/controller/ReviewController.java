...
public class ReviewController {
    ...
    // 이용 후기 게시판 메인 페이지 이동
    @GetMapping("list")
    public String review(HttpSession session, Model model, Pagination pagination) {
        log.info("review()");
    ...
    public String registerPOST(ReviewVO reviewVO, HttpSession session, RedirectAttributes reAttr) {
    ...
    // 이용후기 제목 링크 선택
    @GetMapping("detail")
    public String detail(Model model, Integer reviewId, HttpSession session) {
        log.info("detail()");
        MemberVO member = (MemberVO) session.getAttribute("login");
    ...
    public String delete(Integer reviewId, HttpSession session) {
    	log.info("delete()");
    	MemberVO member = (MemberVO) session.getAttribute("login");
